...
n_model_states = len(model.states)
n_obs = env.observation_space.n
print(f"Learned MDP with {n_model_states} states")
model_state_ids = dict([(v, k) for k, v in enumerate(model.states)])
# potentially have only n_states*2 x |actions|
q_table = np.zeros([env.observation_space.n * n_model_states * 2, env.action_space.n])
...
for i in range(1, num_training_episodes + 1):
    ...
    extended_state = model_state_id * n_obs + state
    if unknown_model_state:
        extended_state += n_model_states * n_obs
    sample = ['Init']
    rl_sample = []
    while not done:
        # print(f"state {state},{model_state_id}: {extended_state}")
        action = env.action_space.sample() if random.random() < epsilon else np.argmax(q_table[extended_state])
        steps += 1
        ...
        next_extended_state = model_state_id * n_obs + next_state
        if unknown_model_state:
            next_extended_state += n_model_states * n_obs
    ...
    if i % 1000 == 0:
        ...
        new_model = run_Alergia(data, automaton_type="mdp")
        new_n_model_states = len(new_model.states)
        print(f"Learned MDP with {new_n_model_states} states")

        print("Updating Q table")
        model = new_model
        n_model_states = new_n_model_states
        model_state_ids = dict([(v, k) for k, v in enumerate(model.states)])
        # potentially have only n_states*2 x |actions|
        q_table = np.zeros([env.observation_space.n * n_model_states * 2, env.action_space.n])
        if goal_reached_frequency > 0:
            replay_traces(rl_samples, model, model_state_ids, q_table)
